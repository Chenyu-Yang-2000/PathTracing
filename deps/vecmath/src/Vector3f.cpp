...
Vector3f Vector3f::clamp(double _min, double _max){
	return Vector3f(
		std::max(std::min(m_elements[0], _max), _min),
		std::max(std::min(m_elements[1], _max), _min),
		std::max(std::min(m_elements[2], _max), _min)
	);
}
...
Vector3f min(const Vector3f &v0, const Vector3f &v1) {
	return Vector3f(std::min(v0.x(), v1.x()), std::min(v0.y(), v1.y()), std::min(v0.z(), v1.z()));
}

Vector3f max(const Vector3f &v0, const Vector3f &v1) {
	return Vector3f(std::max(v0.x(), v1.x()), std::max(v0.y(), v1.y()), std::max(v0.z(), v1.z()));
}

bool operator <= (const Vector3f &v0, const Vector3f &v1) {
    return v0.x() <= v1.x() + 1e-8 && v0.y() <= v1.y() + 1e-8 && v0.z() <= v1.z() + 1e-8;
}

bool operator >= (const Vector3f &v0, const Vector3f &v1) {
    return v1.x() <= v0.x() + 1e-8 && v1.y() <= v0.y() + 1e-8 && v1.z() <= v0.z() + 1e-8;
}